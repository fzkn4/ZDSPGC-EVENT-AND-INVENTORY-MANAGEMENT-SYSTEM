(function () {
  ...
  function collectEquipmentsFromForm() {
    ...
    equipmentsInputs.querySelectorAll(".equipment-row").forEach(function (row) {
      var name = String(
        (row.querySelector(".equipment-name") || {}).value || ""
      ).trim();
      var qtyStr = String(
        (row.querySelector(".equipment-qty") || {}).value || ""
      ).trim();
      var unit = String(
        (row.querySelector(".equipment-unit") || {}).value || ""
      ).trim();
      if (!name) return;
  ...
  if (formAddEvent && eventsList) {
    formAddEvent.addEventListener("submit", function (e) {
      ...
      li.className =
        "list-group-item d-flex align-items-start justify-content-between";
      li.innerHTML =
        "<div>" +
        ...
        escapeHtml(String(location || "TBA")) +
        " • " +
        ...
        "</span>" +
        '<button class="btn btn-sm btn-outline-secondary" data-event-title="' +
        escapeHtml(String(title)) +
        '" data-event-date="' +
        escapeHtml(String(date || "")) +
        '" data-event-location="' +
        ...
        '" data-event-it="' +
        escapeHtml(itAttr) +
        '" data-bs-toggle="modal" data-bs-target="#modalEventDetails"><i class="bi bi-card-text"></i> Details</button>' +
        "</div>";
  ...
  if (formAddItem && inventoryList) {
    formAddItem.addEventListener("submit", function (e) {
      ...
      li.innerHTML =
        ...
        ' <span class="text-muted small">(<span class="inv-borrowed" data-item="' +
        escapeHtml(String(name)) +
        '">0</span> borrowed)</span></div>' +
        '<div class="small text-muted">Category: ' +
        escapeHtml(String(category || "General")) +
        " • Location: " +
        escapeHtml(String(location || "Unassigned")) +
        "</div>" +
        "</div>" +
        '<span class="badge text-bg-primary align-self-center">Qty: <span class="inv-qty" data-item="' +
        escapeHtml(String(name)) +
        '">' +
        escapeHtml(String(quantity || "0")) +
        "</span></span>";
  ...
  if (formAddLoan && loansList && invoicesList) {
    formAddLoan.addEventListener("submit", function (e) {
      ...
      li.innerHTML =
        ...
        qty +
        "</div>" +
        '<div class="small text-muted">Borrower: ' +
        escapeHtml(String(borrower)) +
        (contact ? " • " + escapeHtml(String(contact)) : "") +
        (due ? " • Due: " + escapeHtml(String(due)) : "") +
        "</div>" +
        (notes
          ? '<div class="small">' + escapeHtml(String(notes)) + "</div>"
          : "") +
        "</div>" +
        '<div class="d-flex align-items-center gap-2">' +
        '<button class="btn btn-sm btn-outline-success" data-loan-id="' +
        loanId +
        '" data-item="' +
        escapeHtml(String(item)) +
        '" data-qty="' +
        ...
        '" id="btnReturn_' +
        loanId +
        '"><i class="bi bi-box-arrow-in-left"></i> Return</button>' +
        "</div>";
      ...
      invLi.innerHTML =
        ...
        "</div>" +
        "</div>" +
        '<button class="btn btn-sm btn-outline-secondary" data-invoice-id="' +
        invId +
        '" id="btnOpenInv_' +
        invId +
      ...
      if (returnBtn) {
        returnBtn.addEventListener("click", function () {
          ...
          notify(
            "Item returned: " + escapeHtml(String(item)) + " × " + qty,
            "success"
          );
          updateLowStockBadge();
        });
  ...
  function renderInvoice(data) {
    ...
    invoiceContainer.innerHTML =
      ...
      '<div class="small text-muted">Invoice ID: ' +
      escapeHtml(data.invId) +
      "</div>" +
      "</div>" +
      ...
      '<div class="col-12 col-md-6"><span class="text-muted">Borrower:</span> ' +
      escapeHtml(data.borrower) +
      "</div>" +
      (data.contact
        ? '<div class="col-12 col-md-6"><span class="text-muted">Contact:</span> ' +
          escapeHtml(data.contact) +
          "</div>"
        : "") +
      (data.due
        ? '<div class="col-12 col-md-6"><span class="text-muted">Due:</span> ' +
          escapeHtml(data.due) +
          "</div>"
        : "") +
      "</div>" +
  ...
  if (eventDetailsModal) {
    eventDetailsModal.addEventListener("show.bs.modal", function (ev) {
      ...
      var items = [];
      try {
        if (eqRaw) equipments = JSON.parse(decodeURIComponent(eqRaw));
      } catch (e) {
        equipments = [];
      }
      try {
        if (itRaw) items = JSON.parse(decodeURIComponent(itRaw));
      } catch (e) {
        items = [];
      }
      doc.getElementById("eventDetailsTitle").innerHTML =
        '<i class="bi bi-card-text me-2"></i>' + escapeHtml(title);
      doc.getElementById("eventDetailsInfo").innerHTML =
        "" +
        '<div class="details-inventory">' +
        '<div class="mb-2"><span class="text-fixed-red fw-semibold">DATE</span><div class="ms-3 d-inline-block text-fixed-white">&nbsp; ' +
        escapeHtml(date || "") +
        "</div></div>" +
        (location
          ? '<div class="mb-2"><span class="text-fixed-red fw-semibold">LOCATION</span><div class="ms-3 d-inline-block text-fixed-white">&nbsp; ' +
            escapeHtml(location) +
            "</div></div>"
          : "") +
        (tag
          ? '<div class="mb-2"><span class="text-fixed-red fw-semibold">TAG</span><div class="ms-3 d-inline-block text-fixed-white">&nbsp; ' +
            escapeHtml(tag) +
            "</div></div>"
          : "") +
        ...
        (equipments.length
          ? "<ul>" +
            equipments
              .map(function (e) {
                return (
                  '<li><span class="text-fixed-white">' +
                  escapeHtml(e.name) +
                  "</span>" +
                  (e.quantity
                    ? '<span class="ms-3 text-fixed-white fw-semibold">' +
                      e.quantity +
                      '</span> <span class="text-fixed-white">' +
                      escapeHtml(e.unit || "") +
                      "</span>"
                    : "") +
                  "</li>"
              ...
              .join("") +
            "</ul>"
          : '<div class="text-fixed-white small">None</div>') +
        "</div>" +
        '<div class="col-12 col-md-6">' +
  ...
  if (btnCheckIn && attendanceList) {
    btnCheckIn.addEventListener("click", function () {
      ...
      li.innerHTML =
        ...
        escapeHtml(name) +
        (id
          ? ' <span class="text-muted small">(' + escapeHtml(id) + ")</span>"
          : "") +
        "</div>" +
